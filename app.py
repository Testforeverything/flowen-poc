...
if menu == "Journey Management":
    ...
    st.markdown("###  Avg. Time to Success by Journey")
    ...
    if "status_paid" in df.columns:
        donut = df["status_paid"].value_counts().reset_index()
        fig_donut = px.pie(donut, names="index", values="status_paid", hole=0.4)
