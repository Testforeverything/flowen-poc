...
# ─── DATABASE SETUP ──────────────────────────────────────────
# แก้ connection string ให้ตรงกับ DB ของคุณ
...
# ─── REAL-TIME ALERT ──────────────────────────────────────────
high_risk = df[df["dpd"] > 60].shape[0]
if high_risk:
    st.warning(f"🔔 {high_risk} accounts exceeded 60 days past due!")
...
# ─── EXPORT BUTTONS ──────────────────────────────────────────
def to_excel(df_):
    return df_.to_excel(index=False, engine="openpyxl")
def to_pdf(df_):
    buf = BytesIO()
    doc = SimpleDocTemplate(buf, pagesize=letter)
    style = getSampleStyleSheet()
    elems = [Paragraph("Flowen Debtor Report", style["Title"]), Spacer(1,12)]
    data = [df_.columns.tolist()] + df_.values.tolist()
...
st.sidebar.download_button("⬇️ Excel", data=to_excel(filtered), file_name="report.xlsx",
                          mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
st.sidebar.download_button("⬇️ PDF",   data=to_pdf(filtered),   file_name="report.pdf",
...
if menu=="Risk Overview":
    ...
    fig=px.pie(rd, names="Level", values="Count", hole=0.4, color_discrete_sequence=BRAND)
    ...
    trend=pd.DataFrame({"Date":pd.date_range("2025-07-01",7),"Rec":[1.0,1.2,1.3,1.1,1.5,1.6,1.7]})
    fig=px.line(trend,x="Date",y="Rec",markers=True,color_discrete_sequence=[BRAND[0]])
    st.plotly_chart(fig,use_container_width=True)
    ...
    br.columns=["B","C"]
    fig=px.pie(br,names="B",values="C",hole=0.4,color_discrete_sequence=BRAND)
    st.plotly_chart(fig,use_container_width=True)
    fig2=px.histogram(filtered,x="monthly_income",nbins=30,color_discrete_sequence=[BRAND[1]])
